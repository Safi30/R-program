...
library(wordcloud)
library(syuzhet)
api_key <- "vnqNbp4x7VFIfJHcyosk8f3jn"
api_secret_key <- "oBTBSBBccAFYMKEIaOQh1blQluafibhmIT4bL1oWQ4EU0nOzrj"
access_token <- "1510872664381931523-waeRhmuzDb5EINqHzOBazGqxn6dSgG"
access_token_secret <- "SQAM3mmkX3tTSqAz4k89P8iyhdg8syQzDATMtNZAC0nxL"
twitter_token <- create_token(app = "MTRP1",consumer_key = api_key,consumer_secret = api_secret_key,access_token = access_token,access_secret = access_token_secret,set_renv = TRUE)
...
Gates_app <- Gates_tweets %>% select(source) %>% group_by(source) %>% 
  summarize(count=n())
...
tweets <- Gates_tweets_organic %>% select(text) %>% unnest_tokens(word, text)
...
set.seed(1234)
wordcloud(Gates_tweets_organic$hashtags, min.freq=5, scale=c(3.5, .5), random.order=FALSE, rot.per=0.35,colors=brewer.pal(8, "Dark2"))

#the accounts from which most retweets originate
set.seed(1234)
wordcloud(Gates_retweets$retweet_screen_name, min.freq=3, scale=c(2, .5), random.order=FALSE, rot.per=0.25, colors=brewer.pal(8, "Dark2"))
